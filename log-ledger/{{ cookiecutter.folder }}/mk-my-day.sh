...
create_day () {
    ...
    if [ ! -f $file_day_relpath ]; then
        ...
        underline $heading >> $file_day_relpath
...
toctree covers all days' $filename of this week."
    ...
    underline $heading >> $file_week_relpath
...
toctree covers all weeks' $filename of this month."
    ...
    underline $heading >> $file_month_relpath
...
toctree covers all months' $filename of this year."
    ...
    underline $heading >> $file_year_relpath
...
echo "info: Customizing hermesbaby environment for usage in dairy."

# Set to today's day
export DOC_CFG_SOURCE_DIR=$(cat $dayfile)

# Do not to collide with Hermesbaby in BiomechDevDocs
export DOC_CFG_HTML_LIVE_PORT=2024
