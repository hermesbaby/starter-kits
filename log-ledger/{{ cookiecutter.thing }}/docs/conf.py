...
rediraffe_redirects_candidates = ["redirects.txt"]

rediraffe_redirects = None
for filename in rediraffe_redirects_candidates:
    if os.path.exists(filename):
        full_path = os.path.abspath(filename)
        rediraffe_redirects = full_path
        break
...
extlinks = {
    "jira": (f"{config.CM__URL_QUERY_TICKET}/%s","%s"),
    "user": (f"{config.CM__URL_QUERY_USER}/%s", "%s"),
}
...
traceability_external_relationship_to_url = {
    'jira': f"{config.CM__URL_QUERY_TICKET}/field1",
}
